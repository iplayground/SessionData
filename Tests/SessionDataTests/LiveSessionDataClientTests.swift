...
struct LiveSessionDataClientTests {
  ...
  @Test("Network success should return correct schedule data from remote source") func fetchSchedulesFromNetworkSuccess() async throws {
    // Given: A schedule with sessions on both days
    let expectedSchedule = Schedule(
      day1: [Session(time: "09:00", title: "Keynote", tags: [], speaker: "Tim Cook", description: "Opening keynote")],
      day2: [Session(time: "10:00", title: "SwiftUI", tags: ["UI"], speaker: "John Doe", description: "Advanced SwiftUI")]
    )
  ...
  @Test("Day filtering should return only sessions for the specified day") func fetchSchedulesWithDayFilter() async throws {
    // Given: A schedule with sessions on both days
    let expectedSchedule = Schedule(
      day1: [Session(time: "09:00", title: "Day 1 Session", tags: [], speaker: "Speaker 1", description: "")],
      day2: [Session(time: "10:00", title: "Day 2 Session", tags: [], speaker: "Speaker 2", description: "")]
    )
  ...
  @Test("Should fall back to cache when network request fails") func fallbackToCacheWhenNetworkFails() async throws {
    // Given: Network fails but cache has data
    let cachedSchedule = Schedule(
      day1: [Session(time: "09:00", title: "Cached Session", tags: [], speaker: "Cached Speaker", description: "")],
      day2: []
    )
  ...
  @Test("Should fall back to bundle data when both network and cache fail") func fallbackToBundleWhenNetworkAndCacheFail() async throws {
    // Given: Network fails and cache is empty
    let bundleSchedule = Schedule(
      day1: [Session(time: "09:00", title: "Bundle Session", tags: [], speaker: "Bundle Speaker", description: "")],
      day2: []
    )
  ...
  @Test("Speakers fetch should use complete fallback chain from network to bundle") func fetchSpeakersWithFallbackChain() async throws {
  ...
  @Test("Sponsors fetch should use complete fallback chain from network to bundle") func fetchSponsorsWithFallbackChain() async throws {
  ...
  @Test("Staffs fetch should use complete fallback chain from network to bundle") func fetchStaffsWithFallbackChain() async throws {
    let staffJSON = """
    [{
      "name": "Network Staff",
      "title": "Organizer"
