...
struct FetchStrategyTests {
  ...
  func remoteStrategyNetworkOnly() async throws {
    ...
    do {
      let sessions = try await client.fetchSchedules(day: 1, dataLanguage: .fallback, strategy: .remote)
  ...
  func localOnlyStrategyBundleOnly() async throws {
    ...
    let sessions = try await client.fetchSchedules(day: 1, dataLanguage: .fallback, strategy: .localOnly)
  ...
  func cacheFirstStrategyWithFallback() async throws {
    ...
    let sessions = try await client.fetchSchedules(day: 1, dataLanguage: .fallback, strategy: .cacheFirst)
  ...
  func strategiesWorkWithDifferentLanguages() async throws {
    ...
    for language in DataLanguage.allCases {
      ...
      let sessions = try await client.fetchSchedules(day: 1, dataLanguage: language, strategy: .localOnly)
