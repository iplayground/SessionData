...
struct SessionDataClientTests {
  ...
  @Test("Client should have fetchSpeakers method that returns speaker array") func clientHasFetchSpeakersMethod() async throws {
    let client = SessionDataClient.mock
    
    _ = try await client.fetchSpeakers()
  }
  
  @Test("Client should have fetchSponsors method that returns sponsors data") func clientHasFetchSponsorsMethod() async throws {
    let client = SessionDataClient.mock
    
    _ = try await client.fetchSponsors()
  }
  
  @Test("Client should have fetchStaffs method that returns staff array") func clientHasFetchStaffsMethod() async throws {
    let client = SessionDataClient.mock
    
    _ = try await client.fetchStaffs()
  }
  
  @Test("Schedule filtering should return correct sessions based on day parameter") func scheduleDayFiltering() async throws {
    // Create a test client with known data
    let testSession1 = Session(time: "09:00", title: "Day 1 Session", tags: [], speaker: "Speaker 1", description: "")
    let testSession2 = Session(time: "10:00", title: "Day 2 Session", tags: [], speaker: "Speaker 2", description: "")
