...
struct LiveSessionDataClientErrorTests {
  ...
  func testNetworkFailureFallbackToCache() async throws {
    ...
    let sessions = try await client.fetchSchedules(day: nil, dataLanguage: .fallback)
  ...
  func testAllEndpointsBundleFallback() async throws {
    ...
    let speakers = try await client.fetchSpeakers(dataLanguage: .fallback)
  ...
  func testNetworkErrors() async throws {
    ...
    let sessions1 = try await client1.fetchSchedules(day: 1, dataLanguage: .fallback)
    #expect(!sessions1.isEmpty)

    let sessions2 = try await client2.fetchSchedules(day: 1, dataLanguage: .fallback)  // Use day 1 which we know has data
